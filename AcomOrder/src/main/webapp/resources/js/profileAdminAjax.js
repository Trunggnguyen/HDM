$(document).ready(function(){
	...
	$('.btnCapNhatThongTin').on("click", function(event) {
		...
		var href = "https://acomapp.herokuapp.com/acom/api/profile/"+nguoiDungId;
	...
	function ajaxPostChangePass() {
	    	// PREPATEE DATA
	    	 ...
	    	 $.ajax({
	 			...
	 			url : "https://acomapp.herokuapp.com/acom/api/profile/doiMatKhau",
	 			data : JSON.stringify(data),
	 			success : function(response) {
	 				if(response.status == "success"){
	 					$('#doiMKModal').modal('hide');
	 					alert("Đổi mật khẩu thành công. Bạn phải đăng nhập lại để xác nhận");
	 					location.href = "https://acomapp.herokuapp.com/acom/logout";
	 				} else {
	 			    	$('input').next().remove();
