$(document).ready(function() {
	...
	function ajaxGet(page){
		...
		$.ajax({
			...
			url: "https://acomapp.herokuapp.com/acom/api/san-pham/all" + '?page=' + page,
			success: function(result){
				$.each(result.content, function(i, sanPham){
					var sanPhamRow = '<tr>' +
					                  '<td>' + '<img src="/acom/img/'+sanPham.id+'.png" class="img-responsive" style="height: 50px; width: 50px" />'+'</td>' +
    ...
    function ajaxPostLapTop() {
    	// PREPATEE DATA
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "https://acomapp.herokuapp.com/acom/api/san-pham/save",
    ...
    $(document).on("click",".btnCapNhatLapTop", function(event){
		...
		var href = "https://acomapp.herokuapp.com/acom/api/san-pham/"+sanPhamId;
    ...
    $(document).on("click",".btnXoaSanPham", function() {
		...
		if(confirmation){
		  $.ajax({
			  async:false,
			  type : "DELETE",
			  url : "https://acomapp.herokuapp.com/acom/api/san-pham/delete/" + sanPhamId,
    ...
    $(document).on("click",".btnChiTiet", function() {
    	...
    	var href = "https://acomapp.herokuapp.com/acom/api/san-pham/"+sanPhamId;
		$.get(href, function(sanPham) {
			$('.hinhAnh').attr("src", "/acom/img/"+sanPham.id+".png");
